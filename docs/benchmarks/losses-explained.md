...
> the published F1 is 0.991. Every `--op text` F1 in run 3 understates the
> engine until that line is corrected. Details under
...
| **Cold render** — 41.90 ms vs pdfium-render 10.75, hayro 13.13, mupdf 15.05 | **Diagnosed and largely fixed** (`docs/status/cold-start.md`). The gap was kernel time, not instructions: every benchmark `run` passes `--font-dir`, and the scan of the oracle's hermetic `test_fonts` read 33.8 MB of font files whole to reach two small tables per face. Since 2026-09-06 the scan reads only the `name` and `OS/2` byte ranges — **34.9 MB in 89 `read` calls becomes 1.14 MB in 182**, syscall time 22.0 → 10.1 ms, and the 44-file cold/warm ratio 3.83x → 2.69x, inside the peers' 1.5–3.6x band. 0 changed board rows. hayro bundles its fonts and scans nothing, so its cold column never included this work at all. | **largely fixed** | `docs/status/cold-start.md` §5. |
...
identified candidate is a type: PDFium's `GetCharWidth`
...
`ladder_char_width` (`crates/pdfrum-text/src/object.rs:176-202`) is that
function transcribed exit for exit but returning **`f32`**, keeping the
fraction PDFium truncates. Every width and threshold downstream is therefore
slightly larger on our side, and the first branch of `generates_space` —
`(last_pos + last_width - pos.x).abs() <= threshold` returning false, the
branch that *suppresses* a space — is precisely the comparison a fractional
width moves.

**Magnitude not determined.** Truncating `ladder_char_width` to an integer
is the first fix to try, but this pass diagnosed and did not build, so
whether it is the whole cause or part of it is unmeasured.
...
Until `benches/compare/src/engines/pdfrum.rs:109` emits the `chars` stream's
unicodes rather than `Display`, **every `--op text` F1 in run 3 understates
the engine** — for us and only for us, since each other engine's adapter was
written against whatever that engine calls its text output. The order of
work is: fix the harness, re-measure, then attack the residual generated
space. `mixed_en_uicase` should go to 1.000 on the harness fix alone.
