...
| **`image_ccitt_3bigpreview.pdf` text** — F1 0.994, **0.999** against the `chars` stream | The same spurious generated space, in the vertical CJK region: `Clips Clips` vs `Clips` on line 4, and four blank lines that are a lone space each. | **inferior** | Shares the `ladder_char_width` fix above. |
