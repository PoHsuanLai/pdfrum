...
`docs/api-baseline/` existed and said so; its scope numbers were its own
...
reproducible: `docs/api-baseline/*.txt` (eighteen `cargo public-api`
...
read-only PDFium checkout used as the behaviour oracle. Where a number appears in
...
**§A above was written before `docs/api-baseline/` existed.** §A.11
...
itself follows and that the internal working notes and the divergence
...
**Basis, stated because it is not `cargo public-api`.** ~~`docs/api-baseline/`
...
columns below, from `docs/api-baseline/*.txt` at `c1dec08`
...
>   written up in the open work list (`docs/issues-to-file.md`) rather than mislabelled as
...
>    `mod` without, which is what `docs/api-baseline/README.md:103`
...
>    `docs/api-baseline/` would have said the work was already done.
...
2. **The oracle's own public API takes `double` and narrows at its entry
   point.** In the read-only PDFium checkout:
...
1. **The snapshots.** Every `pub const` and every `-> i32` / `-> i64` /
   `-> u32` / `-> usize` in `docs/api-baseline/*.txt`, all eighteen
...
> recorded here and in the internal working notes beside A41.
>
...
| **A41 char half** — `U+0002` → `U+00AD` in `CharBox.unicode` | The `--txt` dump | **12 rows**, 11 currently passing | **Declined**, and this ruling does not reopen it. |
...
>   step 1; neither was visible before `docs/api-baseline/` existed.
...
> additions and touches nothing else — `docs/api-baseline/pdfrum.txt`
...
> `FormSession` has 34 public items (`docs/api-baseline/pdfrum.txt:201-234`).
...
> **nothing**, and `Event` appears nowhere in `docs/api-baseline/pdfrum.txt`.
...
> the baseline pass by reading `docs/api-baseline/pdfrum-text.txt`), and
...
>   `docs/api-baseline/pdfrum-text.txt` for exactly that reason, which
...
> eighteen (`docs/api-baseline/pdfrum-text.txt`). Measured, the noisiest
...
> the internal working notes–8 rather than fixed here (a format
...
Stale docs are an API bug. Same pass: the internal working notes still
...
> **Landed 2026-09-03 (WP13).** Snapshot drift is `scripts/api-snapshot.nu check`, wired into `scripts/ci.nu` after the doc step; the script's "deliberately NOT a gate" header is now the history of why it was held back until the surface settled. Enum constructibility is `crates/pdfrum/tests/construct_enums.rs`, derived from `docs/api-baseline/pdfrum.txt` and `pdfrum+script.txt`. The pub-const sweep is `scripts/check-pub-consts.nu` with allowlist the internal working notes. `fn_params_excessive_bools = deny` on the facade crate. STYLE.md §4 carries the three sentences (the snapshot is the review; unexported signature types are a build failure; a new public item in a member crate is a review question).
...
| 11 | WP12 docs | After the surface is true. Includes the JavaScript sentence and the stale facade status doc. | rustdoc, README, the internal working notes |
...
the internal working notes is rewritten to the new block, in the same
