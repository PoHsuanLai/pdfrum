...
**D5 — the content generator's color loss is ported verbatim.** Only `rg`/`RG`
are emitted; CMYK, ICC, Indexed, Separation, DeviceN, Lab, CalRGB and all
patterns are dropped (§2.4). This is a genuine fidelity loss in PDFium, but it
is *the* behavior of regenerated pages, and Tier-B comparison of a regenerated
page against the oracle's regenerated page requires matching it exactly.
Recorded loudly in the crate docs and in R13's conformance cluster.
