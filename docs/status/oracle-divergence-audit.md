...
## 10. Reconciliation with the re-adjudication (`docs/status/reopened-declines.md`)

The eleven items this audit left declined were re-adjudicated on 2026-09-02
against the oracle-bug rule, one at a time, each measured against the board
rather than estimated. That document is the authority on those eleven; this
section reconciles this one with it. **Nothing above is deleted.** Where a
claim here was superseded, it is marked below with what replaced it and why.

### 10.1 Corrections to this audit's own record

| where | the claim here | corrected to |
|---|---|---|
| §1 rows **A11**, **A23**, **A24** ("already declined", "Relabel") | all three read as outstanding relabels | **Stale — all three are implemented.** They were relabelled in the first tranche (`e75a834`, `a2464fc`, `4ff2908`; see §8's relabel table, which already lists them). The "already declined" text refers to the *design brief's* D7/D18/D15 pages, not to a live decline, and reads as an open item it is not. Re-adjudication §2.1–2.3. |
| §1 row **A15**, §3 (A15), §7 item 15 | "≥2 rows (`dashed_lines`)" | **Wrong fixture and wrong count: 1 row, `corpus/fx/path/bug_0_length_line.pdf`.** `dashed_lines`'s array is `[6 5 -4 3 2 1]` and has no zero entry at all — it hits the *negative* branch, which is a separate ROBUSTNESS case (§8.4.3.6 gives a negative element no meaning, so matching the oracle is the tie-break, as A16). **DECLINED**, on unreachability rather than cost — see §10.2. |
| §1 row **A25** | "6 rows"; and "PDFium treats *any* Adobe marker as transformed, ignoring the transform byte — a smaller BUG folded in here" | **The folded-in claim is struck: it is a misreading.** `jpeg_transform_` (`libjpeg_scanline_decoder.cpp:117-119`) is read at exactly one place, `:121`, which decides a **3-component** image's output space. The transform byte is honoured one layer down at `jdapimin.c:185-198` (`case 0: JCS_CMYK; case 2: JCS_YCCK`), and pdf.js (`jpg.js:1292-1296`) and `zune-jpeg` (`headers.rs:496-508`) agree. **There is no oracle bug here.** The measured cost is **4 rows carrying a 4-component JPEG, not 6**, and **0** hit the alleged bug. All four sub-items DECLINE; re-adjudication §2.5 has each. |
| §1 row **A41** | "3 rows" | **12 rows, and asymmetric between two halves the single figure concealed**: the buffer half (U+FFFE → U+00AD) is **0 rows**, the char half (U+0002 → U+00AD) is **12**, eleven of them currently passing. **DECLINED as scoped** — not because the sentinel is defensible (a Unicode noncharacter crossing a public C ABI is not), but because the char half pays 12 goldens to change one code point in a stream whose consumers already have `FPDFText_IsHyphen`. The buffer half is free and is the natural follow-on. |
| §1 row **A42** | "2 rows" | **0 rows.** No golden records search results. **IMPLEMENTED** — see §10.3. |
| §1 row **A46** | "4 rows" | **0 rows**, and already applied in the second tranche (§9, `fdc395a`/`58b674e`). §9's own correction ledger notes the measure branch fixed only half of it. |
| §1 row **A71**, §7's "keep" row, and `pdfrum-edit`'s module docs | "Tier-B scores us against the oracle's *regenerated* page, so a fix would score as a regression" | **The premise is false, at three lines.** `pdfium_test` has no save flag, so the oracle's regenerated page does not exist (`conformance/src/saveroundtrip.rs:6-8`, SPEC §11 ruling E7); Tier-B compares our render against a golden of the **original** (`conformance/src/run.rs:339-361`); and the one sweep that regenerates compares two renders of the *same* regenerated bytes, where a content loss "dropped for both" cannot score (`conformance/src/mutation.rs:12-17`, `:41-44`) — and it is a separate command, not on the 1757-row board. **Measured cost 0 rows. OVERTURNED and IMPLEMENTED** — see §10.3. |
| §1 row **A76** | "≥14 rows (`text_form*`)" | **DECLINED.** See re-adjudication §2.10 for the measurement. |

The re-adjudication also settles **A45** (`CountRects` splitting per text
object) and **A55**; both are covered there.

### 10.2 A15 — a prototyped fix that measurement refuted

**This is a negative result, and it is evidence.** It is recorded because the
prototype exists, was correct in unit test, and was wrong in pixels — and
because a future agent who re-reads §8.4.3.6 will otherwise reach for the same
fix.

`cfx_agg_devicedriver.cpp:367-373` substitutes **0.1** for any dash entry
`<= 0.000001f`, which catches a genuine zero along with a negative. §8.4.3.6
says a zero-length dash is a **dot** under a round or projecting-square cap and
**nothing** under a butt cap; a 0.1-unit segment is neither shape. pdf.js hands
the array to `setLineDash` unchanged (`canvas.js:1126-1131`), reaching the same
answer by delegation.

The oracle's own fixture argues the spec against it: `bug_0_length_line.pdf`
draws `[0 10] 0 d` under three caps and labels the first, in visible page text,
**"Butt Cap; no Output"** — where the oracle paints a 20-pixel band at RGB
229-230.

The narrowest faithful fix — substitute for negatives only, let a genuine zero
reach the rasterizer, let the cap draw it — was implemented with both citations
and four tests, including a geometry test asserting `area == 0` under butt and
`area > 100` under round and square. It passed. **The pixel measurement refuted
it:**
...
The mechanism, isolated: `kurbo`'s dasher emits a subpath only where it
accumulates non-zero length, so **18 of 20 zero-length dashes are dropped**
before any cap applies — `[0.0, 10.0]` over a 200-unit line yields 2 subpaths
where `[0.1, 10.0]` yields 20. The two survivors are the path's own endpoints,
which is exactly why an area-based test passed: two dots have area. SSIM moved
0.999905 → 0.981354.

**The row loss is not the reason to decline.** The reason is that the change
does not produce the correct behaviour: it trades a small wrong artefact (a
faint sliver where nothing belongs) for a large one (nine of ten dots missing).
SPEC §12 asks for the narrowest *faithful* fix, and this one is not faithful.

**What a future agent must build**, if it is revisited: replace the
`Dashes`-to-`kurbo` handoff in `normalize_dashes`/`resolve_stroke` with an
engine-side dash walker emitting, per dash position, either a real segment or a
zero-length subpath the rasterizer will cap. Both backends must receive
identical geometry, so it belongs in `pdfrum-render`. The fixture's own text
labels are the acceptance criteria.

### 10.3 What the re-adjudication moved, and what landed

| item | verdict | measured | outcome |
|---|---|---|---|
| **A71** | OVERTURNED | 0 rows | **Implemented.** `expressible_rgb` (`pdfrum-edit/src/content/emit.rs`) now delegates to `ColorValue::to_rgb` for every space instead of returning `None` for all but `DeviceRGB`/`DeviceGray` — mirroring `CPDF_Color::GetRGB`'s own delegation to `cs_->GetRGB(buffer)` (`cpdf_color.cpp:116-127`), which is the conversion PDFium has and never reaches. `rg`/`RG` stay the only colour operators; a **pattern** still writes nothing, which is a genuine limit rather than the oversight. `[oracle-bug]`, `cpdf_pagecontentgenerator.cpp:64-78`, §8.6; **pdf.js does not regenerate page content and so is silent here** — no citation is invented for it. |
| **A42** | OVERTURNED | 0 rows | **Implemented** (`9e9176f`). `find::search` drops the `U+FFFE` sentinel from its haystack and carries an `origins` map back to text offsets, so a word split across a line break is found by a query without the hyphen and the yielded range still covers the break. That is pdf.js's design (`pdf_find_controller.js:131`, `:290-307`). The `U+0002`/`U+FFFE` representation in the **char list is deliberately untouched** — that is A41's char half, declined above at 12 rows. |
| **YCCK** *(new; found while re-adjudicating A25, and not an `[oracle-bug]` — it is ours)* | our defect | 1 row's fidelity | **Implemented.** `decode_dct` pinned `output_space` to CMYK for every 4-component JPEG, and `zune-jpeg` 0.5.15 has no `(YCCK, CMYK)` arm (`worker.rs:63-82` implements only `(YCCK, RGB)`/`(YCCK, RGBA)`), so **every YCCK image returned `CodecRejected` and was silently dropped**. It now reads the declared space and asks a YCCK codestream for YCCK — the four-component identity copy at `worker.rs:41-43` — then applies `ycck_to_cmyk`, a bit-exact port of libjpeg's `ycck_cmyk_convert` (`jdcolor.c:544-587` with `build_ycc_rgb_table`'s coefficients at `:236-250`). `zune`'s YCCK→RGB path is deliberately **not** taken: its baked-in composite bypasses `/Decode` and the colour space, and `/Decode` is where the CMYK inversion must happen (§7.4.8; `cpdf_devicecs.cpp:104-136`, `jpg.js:1275-1279`). The `Adobe_transform == 0` path is unchanged, which is what keeps `bug_1646` and `bug_718762` at 1.0. |

**A correction to the re-adjudication itself.** It calls
`corpus/fx/other/1.pdf` "one currently-failing row". It is **passing**: SSIM
0.99491 clears the 0.99 global floor (`conformance/thresholds.toml`), with
`max_channel_diff: 255` because the image is dropped entirely. The fix
