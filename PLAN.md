...
```bash
cd "${PDFRUM_ORACLE_CHECKOUT:-../pdfium-c++}"
...
## M22 — `libpdfrum`: the library for other languages  *(scoped 2026-09-05; the user asked for the traditional C `libpdfrum` first)*  — NOT STARTED
...
the way it uses `libpdfium`. Nothing below the facade changes.

1. **Owned handles on the facade.** Pages and form sessions that own an
   `Arc<Document>` instead of borrowing it — `Document::page_owned(&Arc<Self>,
   index)`, `FormSession::owned(Arc<Document>)` — because a C caller cannot
   carry a lifetime; errors that carry a stable code beside their message;
   cancellation as the host-set flag M20 phase 3 adds. This is a facade
   fill on its own commit, useful to Rust callers too.
2. **`crates/pdfrum-capi`.** `crate-type = ["cdylib", "staticlib"]` →
   `libpdfrum.{so,dylib,dll}` and `libpdfrum.a`; `pdfrum.h` generated by
   `cbindgen` from the Rust signatures so the header cannot drift; `cargo-c`
   for `pkg-config` and installation. The one policy change: this crate
   alone gets `#![allow(unsafe_code)]`, under a written rule — unsafe only
   at the boundary, every function's safety contract in the header, nothing
   unsafe reaches below the facade. The API is opaque handles and
   functions, PDFium-like in shape and ours in content: `pdfrum_open`
   (bytes, password, error out), `pdfrum_page_count`, `pdfrum_page` (an
   owned handle), `pdfrum_render` into a caller-owned RGBA buffer,
   `pdfrum_text`, `pdfrum_words`, `pdfrum_search`, links, bookmarks,
   metadata, attachments, forms dump and fill, `pdfrum_save`, images,
   markdown, `pdfrum_cancel`, `pdfrum_free`, `pdfrum_version`,
   `pdfrum_error_message`. `#[repr(C)]` only for a few value types (rect,
   word, options). Document handles are thread-safe (`Document: Sync`),
   page handles are not, as in PDFium. Scripts stay a build-time feature,
   off by default. About forty functions; no page-object editing in v1.
3. **Proof.** A C program compiled and run in CI against the built library
   (a C compiler in the test step, not in the library's build — the
   pure-Rust rule is about what we ship); `nm -D` diffed against the
   header so a symbol cannot appear or vanish unnoticed; the header under
   the API-snapshot discipline, a change to it a deliberate commit;
   `docs/design/capi.md` with the rules and the memory contract.
4. **The web.** `crates/pdfrum-wasm` on `wasm-bindgen` (pure Rust): open
   bytes, pages and sizes, render to RGBA/`ImageData`, text, words, search,
   markdown, forms fill, save; TypeScript types from the same source;
   `wasm-bindgen-test` under Node; a size budget; `javascript` off (boa's
   `getrandom` needs its `wasm_js` feature when it is on). The facade
   already checks on `wasm32-unknown-unknown` with every backend, the
   codecs, forms, edit and markdown; `system-fonts` stays off there and
   the bundled Foxit base-14 fonts carry text.
5. **Other languages on the same handles.** UniFFI (pure Rust; Python,
   Swift, Kotlin, Ruby from annotated Rust; its `unsafe` stays in its own
   crates) and PyO3 for a first-class wheel via maturin, when someone
   asks; the C header already covers Go, C#, Zig and the rest.

Not in M22: an `fpdf_*` compatibility shim for relinking PDFium users.
It would be an adoption lever, but PDFium's API leaks its internals
(form handles, bitmap formats, page-object editing) and matching it is a
project of its own; only with a real codebase asking.

**Rules:** the capi crate holds no logic — handles and calls into the
facade, one to one; every dependency question through DEPS.md (`cbindgen`
and `cargo-c` are tools, not dependencies); the board untouched.
**Exit:** `libpdfrum` builds, installs with `pkg-config`, the C test
passes in CI, the header is snapshotted, and the wasm crate renders a
page in Node.
