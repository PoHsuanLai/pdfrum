...
impl Indexed {
    ...
    pub fn to_rgb(&self, comps: &[f32], std_conversion: bool) -> Option<Rgb> {
        ...
        Some(self.base.to_rgb_with(&base_comps, std_conversion))
    }
}
...
mod tests {
    ...
    fn indices_map_through_the_palette() {
        let cs = gray_palette();
        let rgb = cs.to_rgb(&[0.0], false).expect("index 0");
        assert!(rgb.r.abs() < 1e-6);
        let rgb = cs.to_rgb(&[3.0], false).expect("index 3");
    ...
    fn out_of_range_indices_yield_no_colour() {
        let cs = gray_palette();
        assert!(cs.to_rgb(&[-1.0], false).is_none());
        assert!(cs.to_rgb(&[4.0], false).is_none());
        assert!(cs.to_rgb(&[f32::NAN], false).is_none());
    }
    ...
    fn a_short_palette_yields_no_colour_rather_than_reading_past_it() {
        ...
        assert!(cs.to_rgb(&[0.0], false).is_some());
        assert!(cs.to_rgb(&[1.0], false).is_none());
    }
    ...
    fn indices_truncate_toward_zero() {
        ...
        let a = cs.to_rgb(&[2.9], false).expect("2.9");
        let b = cs.to_rgb(&[2.0], false).expect("2.0");
