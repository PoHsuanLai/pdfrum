...
impl Separation {
    ...
    #[must_use]
    pub fn to_rgb(&self, comps: &[f32], std_conversion: bool) -> Option<Rgb> {
        if self.none {
            return None;
        }
        ...
        let Some(func) = &self.tint else {
            ...
            return Some(alternate.to_rgb_with(&broadcast, std_conversion));
        };
        ...
        Some(alternate.to_rgb_with(&results, std_conversion))
    }
}
...
impl DeviceN {
    ...
    pub fn to_rgb(&self, comps: &[f32], std_conversion: bool) -> Option<Rgb> {
        ...
        Some(self.alternate.to_rgb_with(&results, std_conversion))
    }
}
...
impl PatternSpace {
    ...
    pub fn to_rgb(&self, comps: &[f32], std_conversion: bool) -> Option<Rgb> {
        ...
        Some(base.to_rgb_with(comps, std_conversion))
    }
}
...
mod tests {
    ...
    fn none_separations_paint_nothing() {
        ...
        assert!(sep.to_rgb(&[1.0], false).is_none());
    }
    ...
    fn a_separation_without_a_transform_broadcasts_its_tint() {
        ...
        let rgb = sep.to_rgb(&[0.25], false).expect("colour");
    ...
    fn a_separation_with_a_transform_uses_it() {
        ...
        let rgb = sep.to_rgb(&[0.5], false).expect("colour");
    ...
    fn device_n_reads_exactly_its_name_count() {
        ...
        let a = cs.to_rgb(&[0.5, 9.0, 9.0], false).expect("colour");
        let b = cs.to_rgb(&[0.5], false).expect("colour");
    ...
    fn a_pattern_space_without_a_base_has_one_component_and_no_colour() {
        ...
        assert!(cs.to_rgb(&[0.5], false).is_none());
    }
    ...
    fn a_pattern_space_with_a_base_adds_one_component() {
        ...
        assert!(cs.to_rgb(&[0.0, 0.0, 0.0, 0.0], false).is_some());
    }
}
