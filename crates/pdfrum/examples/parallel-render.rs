...
//!
//! The one thing worth knowing is the cache. A [`BuildContext`] holds the
//! decoded fonts, colour spaces and images of a document, and it is reached
//! through `&mut` — so it cannot be *shared*, only owned. `map_init` gives
//! each rayon worker its own, which is the right granularity: a document's
//! fonts are decoded once per thread rather than once per page.

use std::path::Path;
use std::process::ExitCode;
use std::time::Instant;

use pdfrum::{BuildContext, Document, Error, Pixmap, RenderOptions};
use rayon::prelude::*;
...
fn run(input: &Path, scale: f64) -> Result<(), Error> {
    ...
    let serial: Vec<Pixmap> = {
        ...
        pages
            .iter()
            .map(|page| page.render_with(&options, &mut ctx))
            .collect::<Result<_, _>>()?
    };
    let serial_time = started.elapsed();

    let started = Instant::now();
    // `map_init` builds one cache per worker, reused across every page that
    // worker handles.
    let parallel: Vec<Pixmap> = pages
        .par_iter()
        .map_init(BuildContext::new, |ctx, page| {
            page.render_with(&options, ctx)
        })
        .collect::<Result<_, _>>()?;
