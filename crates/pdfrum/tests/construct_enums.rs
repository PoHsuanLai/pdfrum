...
//!
//! The total-count test re-parses those files at runtime and asserts
//! the construction count. A new variant updates the snapshot (the
//! other gate) and then fails this test until a construction line is
//! added.
//!
...
const SNAPSHOT_ENUMS: &[(&str, &str, &str)] = &[
    ...
    // which is why this gate was four short once the baseline was regenerated.
    ("pdfrum.txt", "pdfrum::Update", "Update"),
    (
        "pdfrum-edit.txt",
        "pdfrum_edit::StampPosition",
        "StampPosition",
    ),
    ("pdfrum-edit.txt", "pdfrum_edit::FlattenMode", "FlattenMode"),
    ("pdfrum-edit.txt", "pdfrum_edit::Paint", "Paint"),
    ("pdfrum-edit.txt", "pdfrum_edit::Fill", "Fill"),
    ("pdfrum-edit.txt", "pdfrum_edit::LineCap", "LineCap"),
    ("pdfrum-edit.txt", "pdfrum_edit::LineJoin", "LineJoin"),
    ("pdfrum-edit.txt", "pdfrum_edit::Flattened", "Flattened"),
...
fn construct_edit_variants() -> usize {
    ...
    let _ = AnnotLinkAction::Uri(String::new());
    let _ = AnnotLinkAction::GoTo {
        page: ObjRef::new(1, 0),
...
fn edit_enum_variants_are_constructible() {
    assert_eq!(construct_edit_variants(), 22 + 2 + 5 + 8 + 8 + 10 + 4);
}
...
fn every_public_enum_variant_is_constructible_from_the_facade() {
    ...
    // 363 -> 358 (`AnnotBorderStyle` is now an alias of `pdfrum_doc::ap::BorderStyle`,
    // counted through that crate's own snapshot rather than here).
    ...
    assert_eq!(constructed, 390, "default-feature variant count");
