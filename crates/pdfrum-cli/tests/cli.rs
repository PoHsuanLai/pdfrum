...
fn time_limit_stops_the_command_with_exit_4_and_a_generous_one_changes_nothing() {
    ...
    // Several files under an already-spent budget: open itself refuses each
    // file, so every one is reported and the exit is 4. (`info` walks pages
    // with Document::pages, which soft-stops on a mid-walk deadline without
    // surfacing LimitExceeded — so a 1ms budget can race past exit 4 on a
    // fast runner. A zero budget is spent before the first open check.)
    let several = run(&[
        "--time-limit",
        "0ms",
        "info",
    ...
    let reported = stderr.matches("time limit of 0 s exceeded").count();
    assert_eq!(reported, 2, "one report per file at open: {stderr}");
