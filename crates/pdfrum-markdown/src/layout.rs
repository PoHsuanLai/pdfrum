...
pub fn layout(lines: &[Line], page: Rect) -> String {
    ...
    // Rows: lines grouped by baseline, top to bottom (y grows down the
    // page in the text page's space), then left to right.
    ...
    for row in rows {
        let top = row.first().map_or(0.0, |l| l.bbox.y0);
        if let Some(prev) = previous_bottom {
            // One blank line per row height of empty page between rows.
            #[expect(
                clippy::cast_possible_truncation,
                clippy::cast_sign_loss,
                reason = "clamped to 0..=3 before the cast"
            )]
            let blank = ((top - prev) / row_h).floor().clamp(0.0, 3.0) as u32;
            for _ in 0..blank {
                out.push('\n');
            }
        }
        previous_bottom = Some(row.first().map_or(top, |l| l.bbox.y1));
