...
//! GitHub-flavoured Markdown; [`render_with_images`] links each
//! [`Block::Image`] to a file the caller wrote.
//!
//! [`document_blocks`] is the same reading over every page of a document
//! at once, which is what it takes to see a running header or footer: the
//! same line in the same band on page after page, dropped by [`running`]
//! before either tier reads the page.
//!
