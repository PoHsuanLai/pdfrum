...
*Renamed 2026-09-02, was `pdfrum-raster-vello-gpu` (type `VelloGpuBackend`).*
The crates now take vello's own names — upstream's `vello` **is** the GPU
renderer on `wgpu` — so the bare name belongs here and the CPU wrapper is
...
// `device` and `queue` must come from `pdfrum_raster_vello::wgpu`.
