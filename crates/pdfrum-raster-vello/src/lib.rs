...
//! [`VelloDevice::draw_image`] therefore wraps a non-opaque draw in an
...
impl RasterBackend for VelloBackend {
    ...
    fn new_target(&self, w: u32, h: u32, clear: peniko::Color) -> Self::Device {
        ...
        VelloDevice::new(w, h, base)
    }

    fn new_target_with_backdrop(&self, base: &Pixmap) -> Self::Device {
        VelloDevice::new(base.width(), base.height(), base.clone())
    }
