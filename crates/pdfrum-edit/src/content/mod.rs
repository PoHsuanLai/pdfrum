...
//! lossy, and matching the loss is the requirement", on the premise that
//! conformance compares a regenerated page of ours against a regenerated page
//! of the oracle's, so emitting more would fail as surely as emitting less.
//! That premise was false, the colour loss it excused has been fixed, and the
//! rest stand as genuine limits. See the comment on the module body for the
//! three lines that settle it. The losses, in full:
//!
...
// The three lines behind the A71 correction in the module docs above, each
// read at the source. `pdfium_test` has no save flag, so there is no such
// thing as the oracle's regenerated page
// (`conformance/src/saveroundtrip.rs:6-8`). Tier-B compares our render
// against a golden of the *original* (`conformance/src/run.rs:339-361`). And
// the one sweep that does regenerate compares two renders of the **same**
// regenerated bytes, so a content loss "dropped for both" cannot score either
// way (`conformance/src/mutation.rs:12-17`, `:41-44`) — and it is a separate
