...
impl CharIndex {
    ...
    /// ```
    #[must_use]
    pub fn char_index(&self, text_index: usize) -> Option<usize> {
        let mut remaining = text_index;
        for segment in &self.segments {
            let count = segment.count as usize;
            if remaining < count {
                return Some(segment.index as usize + remaining);
            }
            remaining -= count;
        }
        None
    ...
    /// text does not hold.
    #[must_use]
    pub fn text_index(&self, char_index: usize) -> Option<usize> {
        ...
        for segment in &self.segments {
            ...
            if char_index < start + count {
                return Some(before + (char_index - start));
            }
            before += count;
        }
        None
    ...
    /// failing.
    #[must_use]
    pub fn text_index_at_or_after(&self, char_index: usize) -> Option<usize> {
        let mut before = 0usize;
        for segment in &self.segments {
            let start = segment.index as usize;
            let count = segment.count as usize;
    ...
    #[must_use]
    pub fn text_index_end(&self, char_index: usize) -> usize {
        let mut before = 0usize;
        let mut end = 0usize;
        for segment in &self.segments {
            let start = segment.index as usize;
...
mod tests {
    ...
    fn a_run_with_nothing_stripped_is_one_segment() {
        ...
        for at in 0..5 {
            assert_eq!(index.char_index(at), Some(at));
            assert_eq!(index.text_index(at), Some(at));
        }
        assert_eq!(index.char_index(5), None);
    }
    ...
    fn an_empty_page_has_no_segments() {
        ...
        assert_eq!(index.char_index(0), None);
        assert_eq!(index.text_index(0), None);
    }
    ...
    fn a_stripped_character_splits_the_segments_and_keeps_the_char_index() {
        ...
        assert_eq!(index.char_index(5), Some(7));
        assert_eq!(index.text_index(7), Some(5));
        // The two control characters are in no segment.
        assert_eq!(index.text_index(5), None);
        assert_eq!(index.text_index(6), None);
    }
    ...
    fn leading_stripped_characters_slide_the_first_segment_forward() {
        ...
        assert_eq!(index.char_index(0), Some(1));
    }
    ...
    fn the_charcode_zero_placeholder_is_stripped() {
        ...
        let index = build(&chars);
        assert_eq!(index.text_len(), 5);
        assert_eq!(index.char_index(0), Some(22));
        assert_eq!(index.text_index(22), Some(0));
    }
    ...
    fn the_forward_and_backward_bounds_skip_stripped_characters() {
        ...
        assert_eq!(index.text_index_at_or_after(2), Some(2));
        // And the end bound for character 2 stops after 'b'.
        assert_eq!(index.text_index_end(2), 2);
        assert_eq!(index.text_index_end(3), 3);
        assert_eq!(index.text_index_end(99), 4);
    }
}
