...
The six PDFs in this directory are copied verbatim from the PDFium checkout
...
| `latin_extended.pdf` | 1 | 19213 B | A page of accented Latin text through a non-trivial encoding, so glyph lookup rather than glyph count is what costs. |
...
## Why these six and not the oracle's whole corpus
...
does, one where cross-reference recovery does, and two that are multi-page so
per-document cache reuse has something to reuse. A benchmark whose fixtures
all look alike reports one number six times.
...
extrapolated to one. The corpus that would fix this
(`testing/corpus`) is not part of the checkout. `docs/status/M8.md` records
this as a stated limitation rather than working around it.
